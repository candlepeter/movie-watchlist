const displaySearchResultEl = document.getElementById(
  "display-search-result-el"
);
...
async function submitForm(e) {
  ...
  try {
    const res = await fetch(
      `http://www.omdbapi.com/?apikey=a3487a29&s=${searchInputValue}`,
      {
        method: "GET",
        headers: {
    ...
    displaySearchResultEl.innerText = `Could not fetch`;
  }
}
...
async function addToWatchlist(id) {
  try {
    const res = await fetch(`http://www.omdbapi.com/?apikey=a3487a29&i=${id}`, {
      method: "GET",
      headers: {
        accept: "application/json",
      },
